...
export default function TransferPage() {
  ...
  const handleLookup = async () => {
    ...
    setResolvedName('')
    try {
      const account = await lookupAccount({ data: { accountNumber: toNumber } })
      if (!account) {
        setLookupError('Nomor rekening tujuan tidak ditemukan')
        return
      }
      setResolvedName(account.fullName)
    } catch (err: any) {
      setLookupError(err.message || 'Gagal memeriksa rekening tujuan')
    } finally {
      setLoading(false)
    }
  }
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {transferType === 'internal' && (
                  <button
                    ...
                    disabled={toNumber.length < 10 || loading}
                    style={{
                      ...
                      opacity: toNumber.length < 10 || loading ? 0.5 : 1,
                    }}
                  >
